...
def extract_phase(
    ...
    Returns:
        wrapped phase map (H, W) float64, in [-pi, +pi]. For a sin-based fringe
        (as generated by ui/main_window.py and by the simulation pipeline), this
        returns +phi_true with no sign flip.
    """
    ...
    # arctan2(c, s) on a sin-based fringe gives +phi (matches simulation/reconstruction.py).
    # The fringe generator in ui/main_window.py is sin-based to keep this convention.
