...
def make_tilt(
    ...
    """Tilted plane h(x, y) = slope_x * x + slope_y * y.

    Slopes are dimensionless (mm per mm). The output has mean zero by
    construction because the (X, Y) grid is centered on the origin.
    ...
    slope_x, slope_y : float
        Plane slopes along the X (horizontal) and Y (vertical) axes.
...
def make_step(
    ...
    """Hard step: h = height_mm where x > edge_x_mm, else h = 0.

    `edge_x_mm` is measured from the grid center (positive x is right).
    The edge is intentionally not smoothed; the discontinuity aliases
    when fed through the synthesized-fringes forward model, which is the
    honest behavior for a step-gauge ground truth (matches Chapter 4's
    VLSI step calibration approach).
    ...
    pixel_size_mm : float
    height_mm : float
        Step height. Negative values are allowed and produce a downward
        step on the positive-x side.
    edge_x_mm : float
        X-coordinate of the step edge in mm, relative to grid center.

    Returns
    -------
    ndarray of shape (H, W), dtype float64.
    """
    X, _ = _centered_grid_mm(shape, pixel_size_mm)
    result = np.zeros(shape, dtype=np.float64)
    result[X > edge_x_mm] = height_mm
    return result
...
def make_sphere(
    shape: Tuple[int, int],
    pixel_size_mm: float,
    cap_height_mm: float = 0.5,
    footprint_radius_mm: float = 20.0,
) -> np.ndarray:
    """Spherical cap, centered on the grid.

    The cap is a portion of a sphere of radius `R` that meets the
    surrounding flat at radius `a = footprint_radius_mm`. The sphere
    radius is derived from the cap height `h = cap_height_mm` and the
    footprint radius `a` via the sagitta formula:

        R = (a**2 + h**2) / (2 * h)

    For radial distance `r = sqrt(x**2 + y**2) <= a`:

        z(r) = h - (R - sqrt(R**2 - r**2))

    chosen so that `z(0) = h` (peak at center) and `z(a) = 0` (edge of
    cap meets the surrounding flat).

    Outside the footprint (r > a), z = 0.
    ...
    pixel_size_mm : float
    cap_height_mm : float
        Peak height of the cap at the grid center. Must be > 0.
    footprint_radius_mm : float
        Radius of the cap's circular footprint in mm. Must be > 0.
    ...
    ------
    ValueError
        If `cap_height_mm <= 0` or `footprint_radius_mm <= 0`.
    """
    if cap_height_mm <= 0.0:
        raise ValueError(
            f"cap_height_mm must be > 0; got {cap_height_mm}"
        )
    if footprint_radius_mm <= 0.0:
        raise ValueError(
            f"footprint_radius_mm must be > 0; got {footprint_radius_mm}"
        )

    h = cap_height_mm
    a = footprint_radius_mm
    R = (a * a + h * h) / (2.0 * h)
    ...
    result = np.zeros(shape, dtype=np.float64)
    inside = r_sq <= a * a
    # R**2 - r_sq is >= R**2 - a**2 > 0 within the footprint.
    result[inside] = h - (R - np.sqrt(R * R - r_sq[inside]))
    return result
