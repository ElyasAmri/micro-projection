...
- **Lab-view recovered-surface consistency (NEXT TASK).** The 3D Scene lab view currently renders a recovered surface that **predates the inverse-FPP work**, so it diverges from the Recovered Surface tab. Goal: feed the 3D Scene lab view the SAME inverse-FPP recovered surface, rendered over ground truth, updating live as parameters vary — real-time orientation/recovery intuition. **COORDINATION:** Ilyas's projector swap (Wintech PRO4500, §2) WILL touch the lab view (old projector → new); he hasn't started. Keep his work in a separate `_build_*` method and the recovered-surface logic in its own method; never blind-merge.
...
- **FOV margin for camera tilt (coupled to the projector swap, §2).** The draggable object-FOV grid should be shrunk INSIDE the camera's true 68×55mm capture footprint to allow camera tilt — required margin ~ `object_height × tan(θ_camera)`. Folds in the likely new-projector placement (projector at 0° vertical, camera at angle). **The telecentric camera does NOT acquire perspective bias at an angle** — camera tilt is handled by the two-angle λ_eq (Eq. 2-51) as the θ_camera parameter, NOT a new bias term; there is no bias-arm-inversion concern (the earlier §2 flag was based on a wrong premise — corrected there).
- **B.4 — deterministic serializable reference** still open (unchanged from B.3a close): formalize the seeded B.3a config into a saved/reloadable reference + serialized headline numbers as the hardware-phase validation target.
