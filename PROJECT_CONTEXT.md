...
The user has an existing Jupyter notebook (`notebooks/Fringe_Projection_Python.ipynb`) that implements an end-to-end synthetic simulation. **It works** — recovers a Gaussian bump from simulated fringes with mean error ~10⁻⁵ after DC alignment. Stages 1 through 5, plus **Stage 6 Phase A→B.3a** (the inverse-FPP reconstruction pipeline + the physical beyond-Nyquist wall + the headline dynamic-range showcase) and **Stage 6 B.3b** (STL performance arc + part-agnostic defect-detection demo, see Section 12), have been completed. The notebook is the starting point for refactoring, not a thing to start over.

**North-star (reaffirmed Stage 6 B.3b).** The simulation must handle the inverse-FPP math for the projector AND the camera at **any** orientation — the lab view already parameterizes both arm angles (θ_projector, θ_camera) — and produce correct pipeline output + a functioning Recovered Surface tab for any setup. The **likely default test setup is projector at 0° (vertical) with the camera at the triangulation angle** (the telecentric camera handles tilt via the two-angle λ_eq, Eq. 2-51, with no new bias term — see §2). The Recovered Surface tab should eventually ingest **practical (real-hardware) recovered results** alongside the synthetic ones. The ultimate goal: a simulation faithful enough to the abstract's inverse-FPP proposal (real-time adaptive nulling for in-situ AM defect detection) that hardware integration is smooth.
...
**TENTATIVE mounting plan (NOT finalized) — projector at 0°, camera at the angle.** The likely physical arrangement inverts the current sim: **projector directly above the surface at 0°** (minimizes projection-arm perspective curvature → less to correct) with the **telecentric camera arm carrying the triangulation angle**. ✅ **CORRECTED (Stage 6 B.3b) — no bias-arm-inversion concern.** An earlier note here worried that putting the projector at 0° and the camera at the angle would "shift the bias source" and require re-examining the inverse-FPP/nulling story (§7.14). That was based on a **wrong premise:** the telecentric camera does **NOT** acquire perspective bias when tilted. Camera tilt enters the math purely as the **θ_camera parameter in the two-angle λ_eq (Eq. 2-51)** — it is a triangulation angle, not a new bias term. The non-telecentric **projector arm remains the sole source of the perspective bias** the inverse grating corrects, regardless of which arm carries the triangulation angle. So projector-at-0°/camera-at-angle is fine for the nulling story; the inverse grating still corrects the projector's bias, no re-derivation needed. **Coupled future task — FOV margin for camera tilt:** with the camera tilted, the draggable object-FOV grid should be shrunk INSIDE the camera's true 68×55 mm capture footprint to leave room for the tilt (required margin ~ `object_height × tan(θ_camera)`); fold this in when the placement is finalized. (Tracked in the Stage 6 B.3b carry-forward flags, §12.)
...
| **6 (B.3b)** | **STL performance arc + part-agnostic defect-detection demo. REPLANNED: a believable STL at honest mm scale is sub-Nyquist by design, so it carries the §7.14 DEFECT-DETECTION story, not the dynamic-range headline. Rasterizer byte-exact fixture (perf.0) → vectorize (perf.1) → in-place shared lift (perf.1b) → display-only downsample (perf.2) → readout gated off for STL (demo). 6 commits, 341 tests passing at close.** | No | ✅ Done |
| **6 (B.3b lab-view + FOV presets)** | **Lab-view recovered-surface consistency + selectable FOV-grid presets (committed after `stage-6-b3b-complete`; pushed at close under tag `stage-6-b4-complete`). Lab view wired to the SAME inverse-FPP recovery as the Recovered Surface tab — it was rendering the pre-inverse-FPP forward `run_pipeline` surface → now angle-invariant nulling is visible (§7.18). `_fov_shape` single-source FOV size + preset dropdown with runtime ROI resize; footprint-sourced prism, so a smaller grid buys camera-tilt headroom (§7.19). 4 commits, byte-neutral GUI lifts.** | No | ✅ Done |
| **6 (B.4)** | **Deterministic serializable reference the hardware phase validates against: `ShowcaseConfig` JSON serializer (fills `io_utils`) + single-source core lifts (`steep_region_ratios`, `make_demo_defect`) + committed B.3a reference artifact (config JSON + numbers/array `.npz`) + reload-verify test (integer ratios exact, floats/array atol 1e-8). 4 commits, 357 tests passing. Closed: 8 substantive commits (`961709a` labview.1 → `bb3ee56` B.4.2b) + mid-stage docs (`2b57636`) + docs-close = 10 past `stage-6-b3b-complete`; tagged `stage-6-b4-complete`, pushed at close.** | No | ✅ Done |
...
### 7.16 — Two distinct showcases: synthetic steep dome (beyond-Nyquist validation) vs real STL parts (defect-detection) (Stage 6 B.3a + B.3b)

The inverse-FPP work has **two showcases doing different jobs** — and a correction landed in B.3b: they are NOT "the same headline on a believable part." They tell **different stories**.

- **Synthetic steep dome (B.3a) — the beyond-Nyquist VALIDATION gate.** A `make_steep_dome` golden in the **math-pixel convention** (amplitude ~6000, σ ~60 px) whose flanks push the observed fringe frequency past 0.5 cyc/px. Extreme, tunable, reproducible (seeded); produces clean headline numbers: **decoupling ~42–135×** (observed-frequency reduction in the steep region) and a **steep-region recovery-error improvement of ~10⁴–10¹⁵×**. Surfaces as the live "Steep-region: decoupling Nx, error ratio Mx" readout on the Recovered Surface tab (convention-agnostic ratios; dual-run gated to the dome). Inverse-FPP OFF → steep flanks alias (RMS ~hundreds of mm), ON → recovers to the ~1mm noise floor. This is the **only** convention in which a golden crosses Nyquist, because a believable mm part can't (next bullet).

- **Real STL parts (B.3b) — the DEFECT-DETECTION story (§7.14), NOT the dynamic-range headline.** The original plan ("steep STL = the realistic version of the dome's headline") was **REPLANNED on a physics correction:** a believable STL part at honest mm scale is **sub-Nyquist by design**. A real steep part would need *multi-metre Z within the 68×55 mm FOV* to push the observed fringe frequency past the wall (mm heights at 0.1 mm/px scale per-pixel gradients down 10×). So the STL does **not** carry the dynamic-range headline. What it demonstrates instead is the §7.14 golden-referenced **defect detection**: a matching golden nulls to ~1e-13 (machine precision), and an injected defect survives as the deviation-from-golden in the error map — verified in-GUI on both real parts (pillar block + 12-inch baseplate). Part B's sharp machined edges DO cross Nyquist *locally* (~0.72% of frame), but that is an **edge artifact, not a dynamic-range result** — which is exactly why the dynamic-range readout is **gated OFF for STL** (B.3b-demo commit): an STL part shows "Surface deviation — see Error Statistics (dynamic-range decoupling: steep-dome showcase only)", never a bogus "decoupling Nx" line.

**The dome proves the method against the wall; the STL parts show the method doing believable in-situ AM defect detection.** Two stories, deliberately not conflated.

**B.3-polish legibility (done):** the steep-dome 3D render is dominated by the unit-seam spike (Z≈6000, the §7.12 cosmetic consequence); the B.3-polish commit fits the view's z-max to the p99 of displayed Z and auto-enables Color-by-error on steep-dome entry so the payoff reads visually, not just via the Error Statistics box. Both gated dome-only; STL parts stay honest mm geometry-view.

### 7.17 — Byte-exact guardrail before optimizing a numeric core (Stage 6 B.3b-perf)

Before optimizing `_rasterize_triangles` (the STL→heightmap rasterizer) for performance, B.3b-perf.0 captured a **frozen byte-exact baseline** of its current output and committed it (`tests/fixtures/rasterizer/rasterizer_baseline.npz`, generated once by `scripts/build_rasterizer_fixtures.py`, asserted via `np.array_equal` in `tests/test_rasterizer_fixture.py`). The discipline and why it was necessary:

- **The existing tests would NOT have caught a byte drift.** The rasterizer's `test_stl_loader` cases assert with `atol` 0.1–0.25 plus two trivial exact checks (all-zeros walls; translation invariance) — none pinned a non-trivial pixel value. A vectorization could shift pixels and still pass. "Byte-identical" needed a frozen fixture, not the tolerance tests.
- **The frozen baseline includes the cube's ~1.7764e-15 sub-ULP residual** (the flat-topped box does NOT collapse to exact 0.0 — a barycentric-interp + lift-subtraction artifact). Byte-identical means the residual too: a "cleaner" output (exact 0.0) would be a FAILURE, not an improvement. A normalized-away residual is a regression in disguise.
- **The shared-helper refactor is what extends the gate.** `load_stl_heightmap_full_scale` had no byte-exact test (its tests are atol/origin only). B.3b-perf.1b extracted the lift into a shared `_lift_to_zero` helper used by both loaders, so perf.0's coverage of `load_stl_heightmap` **transitively** validates the arithmetic `_full_scale` also runs — closing the gap without a second fixture.

Lesson, now standing: **before optimizing any numeric core, freeze a byte-exact fixture of its current output (artifacts/residuals included) and prove `np.array_equal` against it; the optimization is then provable, not asserted.** This generalizes the sealed-core `[pipeline] std_err` discipline to any numeric output a refactor touches.

### 7.18 — The lab view shows the SAME inverse-FPP recovery as the Recovered Surface tab; angle-invariant nulling made visible (Stage 6 B.3b-labview)

The 3D-Scene lab view's "recovered" leg was rendering a surface that **predated the inverse-FPP work** — the forward `run_pipeline` producer — so it diverged from the Recovered Surface tab (tab 3), which uses the golden-referenced `run_inverse_fpp`. Both legs now call **one shared producer**, `MainWindow._recover_part_surface` (extracted byte-neutrally in labview.1): tab 3 passes `part = golden + defect`; the lab view passes **`part = golden` (clean, NO defect — defect detection is tab 3's story)** and **`inverse_on=True` hardcoded** (the lab view is always the honest CORRECTED showcase, decoupled from tab 3's inverse-correction checkbox — a checkbox on another tab's UI, not in the lab view's scope). The existing Stage-5 ground-truth/recovered XOR toggle is untouched; only the recovered leg's producer changed.

**The visible result is the §7.14 nulling property surfacing in the lab view: the recovered surface is now ANGLE-INVARIANT.** Before the swap, extreme projector tilt visibly distorted the lab-view surface — the non-telecentric projection-arm perspective bias was flowing into the forward recovery, so the "object" changed shape as the projector angle changed. Inverse-FPP nulls that bias (the affine-in-phase cancellation, §7.14), so the recovered surface no longer changes shape with projector angle. The lab view now tells the honest story: a measurement whose recovered geometry is stable under rig motion, exactly because the bias is corrected. (Verified in-GUI.)

A perf consequence is logged as a carry-forward, not fixed here: with the lab view on the recovered leg, `_refresh_surface_preview` runs `run_pipeline` (its output now consumed only by the Pipeline Stages tab) **and** `run_inverse_fpp` — ~2× the pipeline cost per refresh on the 3D-Scene tab. Gating `run_pipeline` to only run when its consumer needs it is a low-priority follow-up (drag is slightly laggy on Browser-mode STL but acceptable).

### 7.19 — FOV-grid presets are a measurement-planning tool; the camera footprint stays intact underneath (Stage 6 B.3b-fov-preset)

The STL Browser's draggable FOV grid is now a **selectable** size (a dropdown of presets in px @ 0.1 mm/px: `(550,680)` full / `(440,544)` / `(330,408)` / `(220,272)`), threaded through one source of truth, `self._fov_shape` — the slice extraction, the off-part mask, and the drawn minimap ROI all read it, so the captured window and the rectangle that draws it cannot desync. Selecting a preset re-centers on the current FOV center (shrink = zoom in, no jump) and resizes the ROI at runtime (`setSize` under `blockSignals`); the dropdown is browser-only (greyed in synthetic/direct modes — there is no draggable grid there).

**Why it's a planning tool, not just a zoom:** the camera's real 68×55 mm capture footprint stays intact *underneath* a smaller grid. The coverage advisory's viewing prism is sourced from the **camera footprint** (`clip_detection` half-extents 34.0/27.5 mm, derived from `make_viewing_cone_wireframe` — `scene.py`), **NOT** from `SURFACE_SHAPE`/the grid. So shrinking the grid does not shrink the camera's true capture — it selects a sub-window centered inside it, leaving lateral room. At a given camera tilt, a smaller grid therefore lets a **taller** edge sit inside the FOV before it clips off the tilted camera's capture, and the existing height-aware coverage advisory shows that headroom **for free** (no new clip logic). The intuition the tool delivers: where to crop a real object's FOV so its full height is measurable without clipping at a working camera angle.

### 7.20 — Single-source-of-truth lifts for a serializable reference; integer-exact vs float-tolerance (Stage 6 B.4)

The B.4 "deterministic serializable reference" is the artifact the Stage 7 hardware phase validates against, so it must re-derive the headline via **the same code the GUI runs** — a parallel re-implementation that could silently drift is exactly the inconsistency a reference exists to prevent. Two byte-neutral lifts made this honest: the headline ratio math moved from the GUI method `_update_dynamic_range_readout` into core `showcase_metrics.steep_region_ratios` (B.4.2a), and the demo defect moved from the GUI-private `_demo_defect` into core `test_surfaces.make_demo_defect` (B.4.2a2). Both the live GUI readout and the reference generator/test now call **one** function each; the `:.0f` display rounding and label strings stayed in the GUI. The config itself round-trips through `io_utils.ShowcaseConfig` (JSON), which is **GUI-agnostic** — it owns the schema, not the B.3a values, and recomputes `lambda_eq` from the serialized geometry inputs on load (never serializing a derived value that could desync; a deliberate `lambda_eq_override` is rejected at capture).

**Integer-exact vs float-tolerance — the resolution of the cross-platform-float finding.** A determinism audit found the run is byte-reproducible **same-machine** (all RNG is fresh-seeded `default_rng(NOISE_SEED)` per call), but `np.polyfit`/`np.linalg.lstsq` route through BLAS, whose accumulation order differs **across** builds (MKL vs OpenBLAS) — so recovered float arrays carry ~ULP cross-machine drift. The reference therefore splits its assertions: the **integer headline ratios** (decoupling, error-ratio) are pinned **EXACTLY** via the `int(format(x, '.0f'))` display mirror (rounding absorbs ULP), while `std_err`, `max_abs`, and the recovered array are checked with **tolerance** (`atol 1e-8`, the `regression_data.npz` tier — NOT the byte-exact rasterizer-fixture tier). The committed reference values for the B.3a seeded config: **decoupling 219×, error-ratio 37691×** (these are the precise seeded-config point, which legitimately sits outside §7.16's recon-era ~42–135× general range), `std_err 1.298556963203`, `max_abs 29.92299835680` (≈ the 30 px defect amplitude — a physical sanity anchor), recovered `(550,680)`.
...
### Stage 6 B.3b — Done (STL performance arc + part-agnostic defect-detection demo)

**Goal achieved.** B.3b was **replanned mid-stage on a physics correction** (§7.16): the original "steep STL = the dome's headline on a believable part" is impossible — a believable STL at honest mm scale is **sub-Nyquist by design** (a real steep part would need multi-metre Z in a 68×55 mm FOV). So B.3b became two things: (1) make the real parts actually *usable* in the GUI (they rasterize to 4500×4501 / ~20 M-vertex surfaces that choked load and FOV drag), and (2) wire the **defect-detection story (§7.14)** — golden-referenced recovery, matching-geometry null, defect-survives-as-deviation — to be part-agnostic, with the beyond-Nyquist dynamic-range readout gated OFF for STL. 6 commits, **341 tests** passing at close (322 at B.3a close → +19), `[pipeline] std_err` byte-identical at **1.764505e-05** throughout. Tag `stage-6-b3b-complete`, pushed at close.

**The arc:**

| # | Commit | What landed |
|---|---|---|
| perf.0 | `9785210` | **Byte-exact rasterizer regression fixture (§7.17).** Froze `_rasterize_triangles`' current output (`tests/fixtures/rasterizer/rasterizer_baseline.npz` + `scripts/build_rasterizer_fixtures.py` + `tests/test_rasterizer_fixture.py`), incl. the cube's ~1.7764e-15 sub-ULP residual, as the guardrail BEFORE optimizing. `.gitignore` exception for the committed `.npz`. +8 tests. |
| perf.1 | `0f0043c` | **Vectorized `_rasterize_triangles`** — vectorized per-triangle setup + separable barycentric inner block + row-band tiling. Byte-identical (`np.array_equal` vs perf.0, all 4 meshes incl. cube residual; the separable broadcast-add was *proven* bit-identical via the gate, not assumed). Part B: rasterize **27.2s → 18.1s**, rasterizer peak **610MB → 229MB**. |
| perf.1b | `7169e24` | **Shared in-place `_lift_to_zero` helper** (both loaders). Byte-identical. End-to-end full-scale load peak **610MB → 244MB** on part B (killed the ~433MB boolean-index copies + separate output alloc). Gives `load_stl_heightmap_full_scale` a transitive byte-exact gate via perf.0 (which exercises `load_stl_heightmap`). |
| perf.2 | `0a4ebe5` | **Display-only downsample** of the whole-STL GL surface + minimap (`_display_stride`, `_WHOLE_STL_MAX_VERTS=1.5M`, `_MINIMAP_MAX_PX=4M`). Strides only the display arrays; full-res `_stl_full_heightmap` stays the slice source of truth; camera distance from full-res extent (existing ==75/==375 test unchanged); minimap `setRect` at full mm extent (FOV ROI unshifted). Fixed the laggy FOV drag — verified smooth in-GUI. +4 tests. |
| demo | `a8b1ab0` | **Dynamic-range readout gated OFF for STL.** `is_stl = (surface_combo.currentText() == STL_LABEL)` threaded into `_update_dynamic_range_readout`; early return BEFORE any compute sets the label to "Surface deviation — see Error Statistics (dynamic-range decoupling: steep-dome showcase only)". Reason: part B's sharp edges cross Nyquist over ~0.72% of frame and would otherwise print a bogus "decoupling Nx" line on what is the DEFECT-DETECTION story, not the dynamic-range headline. Dome/Flat/Gaussian byte-identical (is_stl=False falls through). +1 test. |
| close | this commit | Docs update (PROJECT_CONTEXT.md + CONVERSATION_SUMMARY.md) + tag `stage-6-b3b-complete`; pushed at close. |

**In-GUI verification (both real parts — `fringe_demo_block_pillars.stl` 160×80mm in `Calibration Artifact/`, `12INCH_Baseplate_HBKU_STL.stl` 450×450mm/~28MB/centered-origin in `CTech_Baseplate_Design_Optimization/`):** defect OFF → golden-vs-golden nulls to ~1e-13 (Max abs error 4.99e-13 mm); defect ON → a clean isolated 2.95 mm hot-spot in the error map, pillars do NOT light up; the STL readout shows the "Surface deviation" text, never a decoupling number. Both parts load + render + drag smoothly post-arc (part B ~18s load, 244MB peak).

**Key decisions locked (cross-referenced to §7):**

| Decision | Rationale |
|---|---|
| Believable STL at honest mm scale is sub-Nyquist; STL = defect-detection story, not dynamic-range headline (§7.16) | A real steep part would need multi-metre Z in a 68×55mm FOV. Part B's sharp-edge Nyquist crossings (~0.72%) are an edge artifact, not a dynamic-range result. |
| Byte-exact fixture before optimizing the rasterizer (§7.17) | The atol 0.1–0.25 tests wouldn't catch a byte drift; freeze the current output (cube residual included) first, then prove np.array_equal. A normalized residual is a failure, not an improvement. |
| Shared `_lift_to_zero` helper for both loaders | Gives `_full_scale` (no byte-exact test of its own) the perf.0 gate transitively; one source of truth for the lift arithmetic. |
| Display downsample is display-only; data path stays full-res | The full-res `_stl_full_heightmap` is the slice source of truth; camera distance + minimap mm-extent computed from full-res so the pinned tests hold and the FOV ROI doesn't shift. |
| Readout gate sits BEFORE all is_steep logic | An is_stl early-return short-circuits cleanly so every dome/Flat/Gaussian path is byte-identical; the gate test asserts absence of the numeric headline, not the disclaimer word. |

**Carry-forward flags (open after this close):**

- **Lab-view recovered-surface consistency — ✅ DONE (B.3b-labview.1/.2, `961709a`/`8d6871f`; see §7.18).** The 3D Scene lab view was rendering a recovered surface that predated the inverse-FPP work, diverging from the Recovered Surface tab. Fixed by wiring both legs to the shared `_recover_part_surface` producer (lab view: `part=golden` clean, `inverse_on=True`); the recovered surface is now angle-invariant (the §7.14 nulling made visible). **COORDINATION (still standing for Ilyas's projector swap, Wintech PRO4500, §2):** the recovered-surface logic lives in `_recover_part_surface` / `_refresh_surface_preview`; the projector body+cone build path is `HardwareScene`/`scene.py` `make_projector_*`. The recon verdict was **DISJOINT** (zero method/data intersection) — keep them separate, never blind-merge.
- **Defect interactivity (B.3b-demo.1) — PARKED (not deferred, parked).** An adjustable/placeable defect adds polish, not truth. The fixed 3 mm Gaussian bump already demonstrates the §7.14 nulling property once; injecting-and-recovering more bumps in a sim is the validation-tautology limit (you only re-confirm the same property). Revisit only if a reviewer wants placeable defects.
- **FOV margin for camera tilt (still open; coupled to the projector swap, §2).** The draggable object-FOV grid should be shrunk INSIDE the camera's true 68×55mm capture footprint to allow camera tilt — required margin ~ `object_height × tan(θ_camera)`. Folds in the likely new-projector placement (projector at 0° vertical, camera at angle). **The telecentric camera does NOT acquire perspective bias at an angle** — camera tilt is handled by the two-angle λ_eq (Eq. 2-51) as the θ_camera parameter, NOT a new bias term; there is no bias-arm-inversion concern (the earlier §2 flag was based on a wrong premise — corrected there). **The §7.19 FOV-preset work is the *manual* version of this intuition** (a selectable smaller grid that buys tilt headroom against the footprint-sourced prism); the open piece is an *automatic* margin derived from `θ_camera`.
- **B.4 — deterministic serializable reference — ✅ DONE (B.4.1 → B.4.2b, `96592e0`/`204754c`/`f1f5f2a`/`bb3ee56`; see §7.20).** The seeded B.3a config is now a saved/reloadable `ShowcaseConfig` (JSON) + a committed reference artifact (config JSON + numbers/array `.npz`) + a reload-verify test that re-derives via the same core (integer ratios exact, floats/array atol 1e-8). The Stage 7 hardware-phase validation target.
- **Lab-view ~2× pipeline cost — NEW open flag (from B.3b-labview.2).** With the lab view on the recovered leg, `_refresh_surface_preview` runs `run_pipeline` (output now consumed only by the Pipeline Stages tab) AND `run_inverse_fpp` — ~2× the pipeline cost per 3D-Scene refresh. Gating `run_pipeline` to only run when its consumer needs it is a low-priority follow-up: in-GUI drag is slightly laggy on Browser-mode STL but acceptable. Don't fix unless drag feel becomes a problem.
- **A.3 — independent-route consistency check** still deferred (validation hygiene; add before the paper if a reviewer angle demands it).
